"""编排核心：进程托管、采集、转写、抽取合并、会议归档。

界面无关——网页控制台（app.py）和桌面应用（desktop.py）都用这一份，
差别只在注入的 `sink`。当初为了做桌面版差点把这段编排复制一份，
两份一定会漂移，所以抽出来。

`sink` 只需实现两个方法：

    async def broadcast(event: dict)      # status / transcript / minutes
    async def log(text: str, level: str)  # 日志行
"""
...
class Sink(Protocol):
    """界面适配器。网页版推 WebSocket，桌面版发 Qt 信号。"""
...
class Managed:
    ...
    def __init__(self, name: str, cmd: list[str], health_url: str, sink: "Sink",
    ...
    async def start(self) -> None:
        if self.external or self.running:
            return
        await self.sink.log(f"[{self.name}] 启动：{' '.join(self.cmd)}")
        ...
        for raw in self.proc.stdout:
            ...
            try:
                asyncio.run_coroutine_threadsafe(
                    self.sink.log(f"[{self.name}] {text}"), loop
                )
            except RuntimeError:
                return
    ...
    async def wait_healthy(self, timeout: float | None = None) -> bool:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            while time.monotonic() < deadline:
                if self.proc is not None and self.proc.poll() is not None:
                    await self.sink.log(f"[{self.name}] 进程已退出，启动失败", "error")
                    return False
                try:
                    resp = await client.get(self.health_url)
                    if resp.status_code < 500:
                        self.healthy = True
                        await self.sink.log(f"[{self.name}] 就绪")
                        return True
                except Exception:
                    pass
        ...
        if self.external:
            ...
            await self.sink.log(f"[{self.name}] 等待就绪超时", "error")
        return False
...
class Engine:
    ...
    async def push_status(self) -> None:
        await self.sink.broadcast(self.status())

    async def push_minutes(self) -> None:
        await self.sink.broadcast({"type": "minutes", **self.minutes.to_json()})
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if mirror:
                ...
                await self.sink.log(f"使用 HuggingFace 镜像 {mirror}")
            ...
            for name, builder in (("llm", llm_cmd), ("asr", asr_cmd)):
                ...
                svc = Managed(name, builder(self.cfg), health, self.sink, external, env)
                self.services[name] = svc
                if external:
                    await self.sink.log(f"[{name}] 外部托管，检查 {health} …")
                else:
                    await svc.start()
                await self.push_status()
            ...
            self._last_persist = 0.0
            await self.sink.log(f"本场会议归档到 meetings/{self.meeting.id}/")

            self.task = asyncio.create_task(self.pipeline())
            self.state = "running"
            await self.push_status()
        except Exception as exc:
            await self.sink.log(f"启动失败：{exc}", "error")
            await self.stop()
    ...
    async def finalize_meeting(self) -> None:
        ...
        if not self.transcript_lines and (recorder is None or recorder.seconds < 1.0):
            ...
            await self.sink.log(f"本场没有任何内容，已丢弃 meetings/{meeting.id}/")
            return
    ...
    async def resolve_model(self, base_url: str, configured: str) -> str:
        ...
        try:
            ...
            await self.sink.log(f"查询模型列表失败，沿用配置值：{exc}")
            return configured
        if not ids:
            return configured
        if configured in ids:
            return configured
        await self.sink.log(f"配置的 {configured} 未加载，改用服务端的 {ids[0]}")
        return ids[0]
    ...
    async def pipeline(self) -> None:
        ...
        self.worker = asyncio.create_task(mn.summarizer(
            ...
            s["overview_every"], on_log=self.sink.log, on_update=self.push_minutes,
        ))
        ...
        async with websockets.connect(ws_url, max_size=None) as ws:
            await self.sink.log(f"已连接转写服务 {ws_url}")
            sender = asyncio.create_task(
                audio.send_audio(ws, dev, loopback, on_log=self.sink.log,
                                 auto_gain=auto_gain,
    ...
    async def receive(self, ws, queue: asyncio.Queue, s: dict) -> None:
        ...
        async for raw in ws:
            ...
            if pending and (len(pending) >= s["min_chars"] or elapsed >= s["max_wait"]):
                await self.sink.log(f"送入摘要 {len(pending)} 字")
                await mn.put_chunk(queue, pending, s["max_pending"], self.sink.log)
